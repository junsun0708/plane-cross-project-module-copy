...
try:
    ...
    print("Error: 'requests' 라이브러리가 필요합니다. 설치: pip install requests")
    sys.exit(1)

# .env 파일을 수동으로 읽는 함수 (dotenv 라이브러리 없을 경우 대비)
...
class PlaneAPI:
    ...
    def _request(self, method: str, path: str, **kwargs) -> Any:
        ...
        for i in range(max_retries):
            ...
            try:
                ...
                # 400 에러일 경우 상세 응답 내용 포함하여 다시 raise
    ...
    def _get_all_pages(self, path: str, per_page: int = 100) -> list[dict]:
        ...
        while True:
            ...
            # Plane API 가 list 를 직접 반환하는 경우도 있음
    ...
    def add_work_items_to_module(self, project_id: str, module_id: str,
        ...
        # Plane v1에서는 'issues' 키를 사용할 수도 있음
        return self._post(
            f"projects/{project_id}/modules/{module_id}/module-issues/",
    ...
    def list_estimates(self, project_id: str) -> list[dict]:
        # 전체 추정 체계(Estimate Categories)를 가져옴
    ...
    def list_estimate_points(self, project_id: str, estimate_id: str) -> list[dict]:
        # 특정 추정 체계의 세부 포인트들(1, 2, 3 등)을 가져옴
